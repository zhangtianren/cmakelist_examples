...
inline int ConservativeRTTEstimate(int rtt) {
  return max(MINIMUM_RTT, min(MAXIMUM_RTT, 2 * rtt));
}
...
int64_t Connection::last_received() const {
  return max(last_data_received_,
             max(last_ping_received_, last_ping_response_received_));
}
