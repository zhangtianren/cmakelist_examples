...
AsyncPacketSocket* BasicPacketSocketFactory::CreateUdpSocket(
    ...
    uint16_t min_port,
    uint16_t max_port) {
  return CreateUdpSocket(address, min_port, max_port, ProxyInfo());
}
...
AsyncPacketSocket* BasicPacketSocketFactory::CreateUdpSocket(
  ...
  if (proxy_info.type == PROXY_SOCKS5) {
    AsyncSocket* tcp_socket =
        socket_factory()->CreateAsyncSocket(address.family(), SOCK_STREAM);
    if (!tcp_socket) {
      delete socket;
      return NULL;
    }

    if (BindSocket(tcp_socket, address, 0, 0) < 0) {
    ...
    socket = new AsyncSocksProxyUDPSocket(
        socket, tcp_socket, proxy_info.address, proxy_info.username, proxy_info.password);
  }
